...
const CategoryIndividualPage : React.FC<{
  ...
  const categoryIndividualDetailURL = 
    `${APP_BASE_URL}/api/category/show/${category_id}`

  const { data : CategoriesListByOperationId , mutate } = useSWR(categoriesListByOperationIdURL , defaultFetcher);
  const { data : categoryIndividualDetail , mutate : mutateIndividualDetail } = useSWR(categoryIndividualDetailURL , defaultFetcher);
  ...
  // Function to download brochure
   const onDownload = async (e : any) => {
    ...
    try {
      // Start download
      const response = await fetch(`${APP_BASE_URL}/api/public/category/download-brochure/${category_id}`, {
        method: 'GET',
        headers: {
          ...
          <div className="mx-auto px-4">
            <CommonContainer>
              <div className="flex justify-between">
                <div>
                  <h1 className="text-4xl font-bold mb-4">{categoryIndividualDetail?.data?.name}</h1>
                  <p className="text-lg">{categoryIndividualDetail?.data?.description}</p>
                </div>
                <Button
                  onClick={onDownload}
        ...
        <CommonContainer>
          <div className="px-4 pb-8">
              <div className="flex md:flex-row flex-col gap-6 relative">
                <CategorySidebar data={categoryIndividualDetail?.data?.children_categories} mutate={mutate} title="Sub Categories"/>
                <div className="flex-1">
                  <div className="grid lg:grid-cols-2 grid-cols-1 gap-4">
                    {
                      categoryIndividualDetail?.data?.products?.length > 0 && 
                      categoryIndividualDetail?.data?.products?.map((product : Record<string,any> , index : number) => {
                        return(
                    ...
                    {
                      categoryIndividualDetail?.data?.products?.length == 0 && 'No Products to show'
                    }
                  </div>
